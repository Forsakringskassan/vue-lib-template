...
We recommend separating Appearance, Logic, and Types into three distinct layers to keep the codebase maintainable. You can find an example component, [`FcCatInfoComponent`](https://github.com/Forsakringskassan/vue-lib-template/blob/main/src/FcCatInfoComponent.vue), which demonstrates how this structure can be applied in practice:

### 1. Type Layer

In the example [cat-types.ts](https://github.com/Forsakringskassan/vue-lib-template/blob/main/src/cat-types.ts), we define _what_ the data is using TypeScript interfaces.

- **Why?** It ensures that both the logic and the component know exactly which fields (e.g., `name`, `age`) exist.

### 2. Logic Layer

In the example [cat-info-logic.ts](https://github.com/Forsakringskassan/vue-lib-template/blob/main/src/cat-info-logic.ts), we create a so-called "composable" (a function that starts with `use...`, e.g., `useCatInfo`).
...
Refer to [FcCatInfoComponent.vue](https://github.com/Forsakringskassan/vue-lib-template/blob/main/src/FcCatInfoComponent.vue) for the actual Vue component. This should be "thin".
